...
while 1:
    ...
    angle_present_goal[1]=angle_goal[1]
    for j in range(2):  #給角度
        n = int(input("num :"))
        angle_goal[j]=n
    print(angle_goal)
    ...
    #寫下馬達的速度 當 angle_goal 大於 angle_present_goal
    if angle_goal[1] > angle_present_goal[1] :
        dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(portHandler, DXL_ID2, ADDR_DXL_GOAL_SPEED,1000)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))
    #寫下馬達的速度 當 angle_goal 小於 angle_present_goal
    if angle_goal[1] < angle_present_goal[1] :
        dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(portHandler, DXL_ID2, ADDR_DXL_GOAL_SPEED,2000)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error))
    ...
    motor2=0
    while 1: 
        # 1Read present position  閱讀現在的位置
        ...
        if real_pulse == 1023:
            i[0]=1
        elif real_pulse == 0 and i[0] == 1 and angle_goal[0] > angle_present_goal[0] :
            circle[0]+=1
            i[0]=0
            angle_pass[0]+=360
        elif real_pulse == 0 and i[0] == 1 and angle_goal[0] < angle_present_goal[0] :
        ...
        # 2Read present position  閱讀現在的位置
        dxl_present_position, dxl_comm_result, dxl_error = packetHandler.read4ByteTxRx(portHandler, DXL_ID2, ADDR_MX_PRESENT_POSITION)
        if dxl_comm_result != COMM_SUCCESS:
            print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
        elif dxl_error != 0:
            print("%s" % packetHandler.getRxPacketError(dxl_error)) 
        ...
        if real_pulse == 1023:
            i[1]=1
        elif real_pulse == 0 and i[1] == 1 and angle_goal[1] > angle_present_goal[1] :
            circle[1]+=1
            i[1]=0
            angle_pass[1]+=360
        elif real_pulse == 0 and i[1] == 1 and angle_goal[1] < angle_present_goal[1] :
        ...
        print("[ID:%d]  PresPos:%s  circle:%d angle:%d\t[ID:%d]  PresPos:%s  circle:%d angle:%d" % (DXL_ID1, real_pulse, circle[0],angle_present+angle_pass[0],DXL_ID2, real_pulse, circle[1],angle_present+angle_pass[1] ))


        #1目標位置停止
        if angle_pass[0]+angle_present-Sensitivity <= angle_goal[0] <= angle_pass[0]+angle_present+Sensitivity  :
        ...
        #2目標位置停止
        if angle_pass[1]+angle_present-Sensitivity <= angle_goal[1] <= angle_pass[1]+angle_present+Sensitivity  :
            dxl_comm_result, dxl_error = packetHandler.write2ByteTxRx(portHandler, DXL_ID2, ADDR_DXL_GOAL_SPEED,0)
            if dxl_comm_result != COMM_SUCCESS: 
                print("%s" % packetHandler.getTxRxResult(dxl_comm_result))
            elif dxl_error != 0:
                print("%s" % packetHandler.getRxPacketError(dxl_error))  
...
#1 Disable Dynamixel Torque  禁用Dynamixel Torque
...
#2 Disable Dynamixel Torque  禁用Dynamixel Torque
