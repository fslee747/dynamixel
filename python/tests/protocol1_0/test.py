...
# 此示例同時將目標位置寫入兩個Dynamixels並重複讀取每個當前位置，直到Dynamixels停止移動。與Syncwrite相關的功能處理多個Dynamixels上Dynamixel控製表中彼此靠近的項目數，例如目標位置和目標速度。
...
BAUDRATE                    = 1000000             # Dynamixel default baudrate : 5760010
...
DXL_MAXIMUM_POSITION_VALUE  = 1021            # and this value (note that the Dynamixel would not move when the position value is out of movable range. Check e-manual about the range of the Dynamixel you use.)
DXL_MOVING_STATUS_THRESHOLD = 100                # Dynamixel moving status threshold 動態像素移動狀態閾值
